...
class UpdateLastLoginToKlaviyoJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    private KlaviyoService $service;

    public function __construct(readonly private User $user, readonly private string $date)
    {
    ...
    public function handle(): void
    {
        ...
        if (!$response->successful()) {
            $this->log(message: 'Unable to update last login to profile', identifier: $this->user->email, response: $response);
            return;
        }
    }
